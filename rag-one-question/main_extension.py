...
def main():
    ...
    parser.add_argument("--gap-threshold", type=float, default=0.02,
                    help="If top-1 and top-2 scores are closer than this, return 'too unsure'")#Added for the extension
    ...
    for q in questions:
        ...
        if is_uncertain(results, gap_threshold):
            ...
            print(f"- Top-1: score={top1[1]:.3f} | {top1[2].doc_id} (chunk {top1[2].chunk_id})")
            print(f"- Top-2: score={top2[1]:.3f} | {top2[2].doc_id} (chunk {top2[2].chunk_id})")
